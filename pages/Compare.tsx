...
import { getCandidates, getJobRoleById, getJobRoles } from '../services/storageService';
import { Candidate, SavedJobRole } from '../types';
import { Search, X, Plus, Trash2, Briefcase, ChevronDown, Check } from 'lucide-react';
...
const Compare: React.FC = () => {
  ...
  useEffect(() => {
      ...
      if (idsParam) {
          const ids = idsParam.split(',').filter(id => allCandidates.some(c => c.id === id));
  ...
  const searchResults = allCandidates.filter(c => {
      ...
      return c.fullName.toLowerCase().includes(searchQuery.toLowerCase()) || 
             c.currentData.skills.some(s => s.toLowerCase().includes(searchQuery.toLowerCase()));
  });
    ...
    <div className="space-y-6 h-full flex flex-col pb-10">
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-4 flex-shrink-0">
                 ...
                 <div className="relative">
                    <select 
                        className={clsx(
                            ...
                            jobRole ? "bg-indigo-50 border-indigo-200 text-indigo-700" : "bg-white border-gray-300 text-gray-700"
                        )}
                        value={jobRole?.id || ''}
                    ...
                    <Briefcase className={clsx("absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4", jobRole ? "text-indigo-500" : "text-gray-500")} />
                    <ChevronDown className={clsx("absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4", jobRole ? "text-indigo-500" : "text-gray-500")} />
                 </div>
            </div>
        </div>
        ...
        <div className="relative w-full lg:w-96" ref={dropdownRef}>
             <div className="relative">
                ...
                <input
                    type="text"
                    placeholder={selectedIds.length >= 3 ? "Max candidates selected" : "Search to add candidate..."}
                    value={searchQuery}
                    onChange={(e) => {
                             ...
                             >
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-gray-500 truncate max-w-[200px]">
                                        {c.currentData.experience?.[0]?.role || c.currentData.skills[0] || 'No Title'}
                                    </div>
                                </div>
      ...
      {jobRole && (
          <div className="bg-white border-l-4 border-indigo-500 shadow-sm rounded-r-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-fade-in flex-shrink-0">
               <div>
                   <div className="text-xs font-bold text-indigo-600 uppercase tracking-wider mb-1">Comparison Context</div>
                   <h3 className="text-gray-900 font-bold">{jobRole.title}</h3>
                   <p className="text-gray-500 text-sm mt-1 max-w-3xl line-clamp-1">{jobRole.description || "Comparing based on skill overlap and experience requirements."}</p>
               </div>
               <div className="flex gap-6 text-sm flex-shrink-0">
                   <div>
                       <span className="text-gray-400 text-xs block">Min Exp</span>
                       <span className="font-semibold text-gray-900">{jobRole.minExperience} Yrs</span>
                   </div>
                    <div>
                       <span className="text-gray-400 text-xs block">Target Skills</span>
                       <span className="font-medium text-indigo-600">{jobRole.requiredSkills.length} Defined</span>
                   </div>
               </div>
          </div>
      )}
      ...
      {selectedCandidates.length > 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-auto">
            <table className="w-full text-left border-collapse table-fixed min-w-[800px]">
                <thead>
                    <tr>
                        ...
                        {selectedCandidates.map(c => (
                            <th key={c.id} className="p-4 border-b border-gray-200 w-1/3 relative group bg-white">
                                ...
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <div>
                                        <div className="text-lg font-bold text-gray-900 truncate">{c.fullName}</div>
                         ...
                         {[...Array(3 - selectedCandidates.length)].map((_, i) => (
                            <th key={`empty-${i}`} className="p-4 border-b border-gray-200 w-1/3 bg-gray-50/30">
                ...
                <tbody className="divide-y divide-gray-100">
                    {/* Experience Row */}
                    <tr>
                        <td className="p-4 bg-gray-50 text-sm font-medium text-gray-700 sticky left-0 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">Experience</td>
                        {selectedCandidates.map(c => {
                            const exp = c.currentData.totalExperienceYears || 0;
                            const meetsExp = jobRole ? exp >= jobRole.minExperience : true;
                            return (
                                <td key={c.id} className="p-4 align-top bg-white">
                                    <div className="flex items-center gap-2">
                                        <span className={clsx("font-semibold text-lg", meetsExp ? "text-green-600" : "text-red-500")}>{exp}</span> 
                                        <span className="text-gray-600">Years</span>
                                        {jobRole && (
                                            meetsExp 
                                            ? <Check className="w-4 h-4 text-green-500" />
                                            : <span className="text-xs text-red-400">(Req: {jobRole.minExperience}+)</span>
                                        )}
                                    </div>
                                </td>
                            );
                        })}
                        {[...Array(3 - selectedCandidates.length)].map((_, i) => <td key={`e-${i}`} className="bg-gray-50/30"></td>)}
                    </tr>
                    ...
                    <tr>
                        <td className="p-4 bg-gray-50 text-sm font-medium text-gray-700 sticky left-0 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                            <div>Skills</div>
                            {jobRole && <div className="text-xs text-gray-400 font-normal mt-1">{jobRole.requiredSkills.length} Required</div>}
                        </td>
                        {selectedCandidates.map(c => (
                            <td key={c.id} className="p-4 align-top bg-white">
                                <div className="flex flex-wrap gap-1">
                                    {c.currentData.skills.map(s => {
                                        ...
                                        const isMatch = jobRole?.requiredSkills.some(req => s.toLowerCase().includes(req.toLowerCase()));
                                        return (
                                            <span 
                                                key={s} 
                                                className={clsx(
                                ...
                                {jobRole && (
                                    <div className="mt-3 pt-3 border-t border-gray-100">
                                        <div className="text-xs font-semibold text-red-400 mb-1">Missing Required:</div>
                                        <div className="flex flex-wrap gap-1">
                                            {jobRole.requiredSkills
                                                .filter(req => !c.currentData.skills.some(s => s.toLowerCase().includes(req.toLowerCase())))
                                                .map(missing => (
                                                    <span key={missing} className="px-1.5 py-0.5 bg-red-50 text-red-600 text-xs rounded border border-red-100 opacity-75">
                                                        {missing}
                                                    </span>
                                                ))
                                            }
                                            {jobRole.requiredSkills.every(req => c.currentData.skills.some(s => s.toLowerCase().includes(req.toLowerCase()))) && (
                                                <span className="text-xs text-green-500 font-medium flex items-center gap-1"><Check className="w-3 h-3"/> All matched</span>
                                            )}
                                        </div>
                                    </div>
                                )}
                         ...
                         {[...Array(3 - selectedCandidates.length)].map((_, i) => <td key={`s-${i}`} className="bg-gray-50/30"></td>)}
                    </tr>

                    {/* Company Row */}
                    <tr>
                        <td className="p-4 bg-gray-50 text-sm font-medium text-gray-700 sticky left-0 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">Recent Company</td>
                        {selectedCandidates.map(c => (
                            <td key={c.id} className="p-4 align-top text-sm text-gray-800 bg-white">
                                <div className="font-medium">{c.currentData.experience?.[0]?.company || "N/A"}</div>
                                <div className="text-xs text-gray-500">{c.currentData.experience?.[0]?.duration}</div>
                            </td>
                        ))}
                        {[...Array(3 - selectedCandidates.length)].map((_, i) => <td key={`c-${i}`} className="bg-gray-50/30"></td>)}
                    </tr>

                    {/* Education Row */}
                     <tr>
                        <td className="p-4 bg-gray-50 text-sm font-medium text-gray-700 sticky left-0 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">Education</td>
                        {selectedCandidates.map(c => (
                            <td key={c.id} className="p-4 align-top text-sm text-gray-600 bg-white">
                                <div className="font-medium text-gray-900">{c.currentData.education?.[0]?.degree || "N/A"}</div>
                                <div className="text-xs text-gray-500">{c.currentData.education?.[0]?.institution}</div>
                            </td>
                        ))}
                         {[...Array(3 - selectedCandidates.length)].map((_, i) => <td key={`ed-${i}`} className="bg-gray-50/30"></td>)}
                    </tr>

                    {/* Project Row */}
                    <tr>
                         <td className="p-4 bg-gray-50 text-sm font-medium text-gray-700 sticky left-0 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">Key Project</td>
                         {selectedCandidates.map(c => (
                            <td key={c.id} className="p-4 align-top text-sm text-gray-600 bg-white">
                                {c.currentData.projects?.[0]?.name ? (
                                    <>
                                        <div className="font-medium text-indigo-700">{c.currentData.projects[0].name}</div>
                                        <div className="text-xs mt-1 line-clamp-3">{c.currentData.projects[0].description}</div>
                                    </>
                                ) : "N/A"}
                            </td>
                        ))}
                        {[...Array(3 - selectedCandidates.length)].map((_, i) => <td key={`p-${i}`} className="bg-gray-50/30"></td>)}
                    </tr>
                </tbody>
            </table>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-gray-400 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50">
             <div className="p-8 text-center">
                 ...
                 <p className="text-gray-500 mt-1 max-w-sm mx-auto">Use the search bar above to add candidates or select from the Role Match page.</p>
             </div>
          </div>
      )}
    </div>
