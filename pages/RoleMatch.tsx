...
import { getCandidates, getJobRoles, addJobRole, deleteJobRole, updateJobRole } from '../services/storageService';
import { analyzeCandidateMatch, generateInterviewQuestions } from '../services/geminiService';
import { Candidate, SavedJobRole, InterviewQuestion } from '../types';
import { Search, Briefcase, CheckCircle, XCircle, Loader2, Check, X, ArrowRight, Plus, Trash2, ChevronLeft, Save, AlertCircle, Mail, FileText, BrainCircuit, MessageSquare, Edit2 } from 'lucide-react';
...
const RoleMatch: React.FC = () => {
  ...
  const [results, setResults] = useState<Array<{candidate: Candidate, score: number, analysis: string, matchedSkills: string[], missingSkills: string[]}>>([]);
  ...
  const validateForm = (): boolean => {
      ...
      if (formData.minExperience < 0) newErrors.experience = "Experience cannot be negative";
  ...
  const handleAnalyze = async (role: SavedJobRole) => {
    ...
    const potentialMatches = allCandidates.filter(c => {
        ...
        const hasSkill = skillsList.some(req => cSkills.some(cs => cs.includes(req)));
        // Relaxed exp check for broad matching
        return hasSkill || (c.currentData.totalExperienceYears || 0) >= (role.minExperience - 2); 
    });
    ...
    const analysisResults = await Promise.all(candidatesToAnalyze.map(async (c) => {
  ...
  const handleEmailCandidate = (e: React.MouseEvent, candidate: Candidate) => {
    ...
    const subject = encodeURIComponent(`Interview Invitation: ${selectedRole.title}`);
...
Hiring Team`
  ...
  const handleGenerateInterview = async (e: React.MouseEvent, candidate: Candidate) => {
    e.stopPropagation();
    ...
    try {
        const qs = await generateInterviewQuestions(candidate, selectedRole?.description ? `${selectedRole.title}: ${selectedRole.description}` : selectedRole?.title);
  ...
  if (view === 'create') {
      return (
        <div className="max-w-2xl mx-auto space-y-6 pb-20">
             <button onClick={() => { setView('list'); setEditingRoleId(null); }} className="flex items-center text-gray-500 hover:text-indigo-600 transition-colors">
                <ChevronLeft className="w-5 h-5 mr-1" /> Back to Roles
            </button>
            
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    <h2 className="font-bold text-lg text-gray-800">{editingRoleId ? 'Edit Job Role' : 'Define New Job Role'}</h2>
                </div>
                <div className="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            className={clsx(
                                "w-full rounded-lg border p-3 bg-white text-gray-900 focus:ring-2 focus:ring-indigo-500 outline-none transition-all",
                                errors.skills ? "border-red-300 focus:border-red-500" : "border-gray-300"
                            )}
                        />
                         {errors.skills && <p className="text-xs text-red-500 mt-1 flex items-center"><AlertCircle className="w-3 h-3 mr-1"/>{errors.skills}</p>}
                ...
                <div className="px-6 py-4 bg-gray-50 flex justify-end gap-3">
                     <button 
                        onClick={() => { setView('list'); setEditingRoleId(null); }}
                    ...
                    >
                        <Save className="w-4 h-4" /> {editingRoleId ? 'Update Role' : 'Save Role'}
                    </button>
                </div>
            </div>
        </div>
      ...
      return (
        <div className="max-w-5xl mx-auto space-y-8 pb-20">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Role Match Simulator</h1>
                    <p className="text-gray-500">Manage job roles and find matching candidates.</p>
                </div>
                <button 
                    onClick={() => {
                        setEditingRoleId(null);
                ...
                {savedRoles.map(role => (
                    ...
                    >
                        <div>
                            ...
                            <h3 className="text-lg font-bold text-gray-900 mb-1 group-hover:text-indigo-600 transition-colors">{role.title}</h3>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-gray-100 text-sm">
                            <span className="text-gray-600 font-medium">{role.minExperience}+ Years Exp</span>
                            <span className="text-gray-400 text-xs">Created {new Date(role.createdAt).toLocaleDateString()}</span>
                        </div>
                    </div>
                ))}
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-20 relative">
       {/* Header / Back */}
        <div className="flex items-center justify-between">
             <button onClick={() => setView('list')} className="flex items-center text-gray-500 hover:text-indigo-600 transition-colors font-medium">
                <ChevronLeft className="w-5 h-5 mr-1" /> Back to Roles
            </button>
            <div className="flex gap-2">
                ...
                >
                   {isAnalyzing && <Loader2 className="w-4 h-4 animate-spin" />} Refresh Matches
                </button>
            </div>
        </div>
        ...
        {selectedRole && (
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 mb-2">{selectedRole.title}</h1>
                         <div className="flex flex-wrap gap-2 mb-3">
                            {selectedRole.requiredSkills.map(skill => (
                                <span key={skill} className="px-2.5 py-0.5 bg-indigo-100 text-indigo-800 rounded text-xs font-medium">
                                    {skill}
                                </span>
                            ))}
                        </div>
                         <p className="text-gray-600 text-sm max-w-3xl">{selectedRole.description || "No additional description provided."}</p>
                    </div>
                    <div className="flex flex-col items-end min-w-[120px]">
                        <span className="text-sm text-gray-500 uppercase tracking-wider font-semibold mb-1">Required Exp</span>
                        <span className="text-xl font-bold text-gray-900">{selectedRole.minExperience} Years</span>
                    </div>
                </div>
            </div>
        )}
        ...
        <div className="space-y-4">
            {isAnalyzing ? (
                <div className="flex flex-col items-center justify-center py-20 text-gray-500">
                     ...
                     <p className="font-medium">AI is analyzing candidates for this role...</p>
                </div>
            ) : results.length > 0 ? (
                 <div className="grid grid-cols-1 gap-4">
                    <div className="flex justify-between items-center px-2">
                        <span className="text-sm text-gray-500 font-medium">{results.length} Candidates Found</span>
                        <span className="text-xs text-gray-400">Select up to 3 to compare</span>
                    </div>
                    
                    {results.map(({ candidate, score, analysis, matchedSkills, missingSkills }) => {
                        ...
                        return (
                            <div 
                                ...
                                className={clsx(
                                    ...
                                    isSelected ? "border-indigo-600 ring-1 ring-indigo-600" : "border-gray-200 hover:border-indigo-300"
                                )}
                            >
                                {/* Checkbox overlay */}
                                ...
                                <div className="flex flex-col sm:flex-row justify-between items-start pr-12 gap-4">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div>
                                            <h3 className="text-lg font-bold text-gray-900 group-hover:text-indigo-600 transition-colors">{candidate.fullName}</h3>
                                            <div className="flex items-center gap-2 text-sm text-gray-500 mt-1">
                                                <span>{candidate.currentData.totalExperienceYears || 0} Yrs Exp</span>
                                                <span className="w-1 h-1 rounded-full bg-gray-300"></span>
                                                <span className="truncate max-w-[200px]">{candidate.currentData.experience?.[0]?.role || "N/A"}</span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <h4 className="text-xs font-semibold text-gray-500 uppercase mb-2 flex items-center gap-1">
                                            <CheckCircle className="w-3 h-3 text-green-500" /> Matched Skills
                                        </h4>
                                        <div className="flex flex-wrap gap-1.5">
                                            {matchedSkills && matchedSkills.length > 0 ? matchedSkills.map(s => (
                                                <span key={s} className="px-2 py-1 bg-green-50 text-green-700 rounded text-xs border border-green-100">
                                                   {s}
                                                </span>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-1.5">
                                            {missingSkills && missingSkills.length > 0 ? missingSkills.map(s => (
                                                <span key={s} className="px-2 py-1 bg-red-50 text-red-700 rounded text-xs border border-red-100">
                                                    {s}
                                                </span>
                                ...
                                <div className="mt-4 pt-4 border-t border-gray-100 flex gap-3">
                                    ...
                                    >
                                        <BrainCircuit className="w-4 h-4" /> Generate Interview Guide
                                    </button>
                                </div>
                            </div>
                        );
                 ...
                 <div className="flex flex-col items-center justify-center py-20 text-gray-400 bg-white rounded-xl border border-dashed border-gray-200">
                    ...
                    <p className="font-medium">No candidates found matching this profile.</p>
                    <p className="text-sm">Try adjusting the role requirements or adding more candidates.</p>
                </div>
            )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl animate-scale-up">
                <div className="flex justify-between items-center p-6 border-b border-gray-100">
                    <div>
                        <h3 className="text-xl font-bold text-gray-900">Interview Guide</h3>
                        <p className="text-gray-500 text-sm mt-1">For {activeInterviewCandidate.fullName} • {selectedRole?.title}</p>
                    </div>
                    <button onClick={() => setActiveInterviewCandidate(null)} className="p-2 hover:bg-gray-100 rounded-full text-gray-500">
                        <X className="w-6 h-6" />
                    </button>
                </div>
                                    ...
                                    <div className="bg-gray-50 p-4 border-b border-gray-100 flex gap-3">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="flex gap-2 mt-2">
                                                <span className="px-2 py-0.5 bg-white border border-gray-200 rounded text-xs font-medium text-gray-600 uppercase tracking-wide">{q.type}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="p-4 bg-white space-y-4">
                                        <div className="text-sm text-gray-600">
                                            <span className="font-medium text-gray-900 block mb-1">Context:</span>
                                            {q.context}
                                        </div>
                                        {q.answerGuide && (
                                            <div className="p-3 bg-green-50 rounded-lg border border-green-100">
                                                <div className="flex items-start gap-2">
                                                    ...
                                                    <div className="text-sm text-green-800">
                                                        <span className="font-bold block text-green-900 mb-1">Good Answer Guide:</span>
                                                        {q.answerGuide}
                                                    </div>
                                                </div>
                                            </div>
