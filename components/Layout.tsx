    ...
    <div className="flex h-screen bg-gray-50">
      ...
      >
        <div className="h-16 flex items-center justify-between px-4 border-b border-gray-200">
          <div className={clsx("flex items-center gap-2 font-bold text-xl text-primary transition-opacity duration-200", isCollapsed && "justify-center w-full")}>
             <BarChart2 className="w-8 h-8 flex-shrink-0" />
             {!isCollapsed && <span className="whitespace-nowrap transition-opacity">RIP AI</span>}
          </div>
        </div>
        ...
        <div className="p-4 pb-2">
             {!isCollapsed && <div className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 px-2 animate-fade-in">Menu</div>}
        ...
        <nav className="flex-1 px-3 space-y-1 overflow-y-auto overflow-x-hidden">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={clsx('w-5 h-5 flex-shrink-0', isActive ? 'text-indigo-700' : 'text-gray-400 group-hover:text-gray-600')} />
        ...
        <div className="p-4 border-t border-gray-200 overflow-hidden">
            <div className={clsx("flex items-center gap-3 mb-4 transition-all", isCollapsed ? "justify-center" : "px-2")}>
            ...
            <div className="relative bg-white w-64 max-w-[85vw] h-full shadow-2xl flex flex-col animate-slide-in-left">
                ...
                <div className="flex-1 overflow-y-auto py-4 px-4 space-y-1">
                     ...
                     {navItems.map((item) => {
                        ...
                        >
                            <item.icon className={clsx('w-5 h-5', isActive ? 'text-indigo-700' : 'text-gray-400')} />
                            {item.label}
                        </Link>
                        );
                    })}
