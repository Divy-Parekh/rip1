  ...
  const filteredCandidates = useMemo(() => {
    ...
    return candidates
      .map((c) => {
        ...
        const matches = queryParts.filter((part) =>
          skills.some((s) => s.includes(part)),
        );

        // Simple rule-based ranking score
        const expScore = Math.min(
          (c.currentData.totalExperienceYears || 0) * 5,
          30,
        );
        const recencyScore = 20;
        const totalScore =
          queryParts.length > 0
            ? (matches.length / queryParts.length) * 50 +
              expScore +
              recencyScore
            : 0;
      ...
      .filter((c) => {
        if (minExp > 0 && (c.currentData.totalExperienceYears || 0) < minExp)
          return false;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">
            Search by skills (e.g., &quot;React Node Mongo&quot;)
          </p>
        </div>
        <Link
          to="/ingest"
          className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 text-sm font-medium shadow-sm transition-colors flex items-center gap-2"
        >
            ...
            {filteredCandidates.map((c) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-wrap gap-1">
                    {c.currentData.skills.slice(0, 3).map((s) => (
                      <span
                        key={s}
                        className="px-2 py-0.5 text-xs bg-gray-100 text-gray-700 rounded-full border border-gray-200 group-hover:border-indigo-100 group-hover:bg-white"
                      >
                        {s}
                      </span>
                    ))}
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {searchQuery ? (
                    <span
                      className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${c.matchScore && c.matchScore > 70 ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}
                    >
