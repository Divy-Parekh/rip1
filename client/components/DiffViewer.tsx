  ...
  if (!hasChanges) {
    return (
      <div className="mb-4 border border-gray-200 rounded-lg overflow-hidden">
        {title && (
          <div className="bg-gray-50 px-4 py-2 border-b border-gray-200 font-medium text-gray-700 text-sm">
            {title}
          </div>
        )}
        <div className="p-4 text-gray-500 italic text-sm bg-white">
          No changes detected.
        </div>
      </div>
    );
        ...
        {diffs.map((part, index) => {
          if (!part.added && !part.removed) {
            ...
            return (
              <div
                key={index}
                className="px-4 py-1 text-gray-500 flex border-b border-gray-50 last:border-0 hover:bg-gray-50"
              >
                <span className="w-6 inline-block text-center select-none opacity-50">
                  {" "}
                </span>
          ...
          const colorClass = part.added
            ...
            : "bg-red-50 text-red-800 line-through decoration-red-300 opacity-70";
          ...
          return (
            <div
              key={index}
              className={clsx(
                "px-4 py-1 flex border-b border-white/50",
                colorClass,
              )}
            >
              <span className="w-6 inline-block text-center select-none font-bold opacity-70">
                {symbol}
              </span>
              <span className="whitespace-pre-wrap break-words">
                {part.value}
