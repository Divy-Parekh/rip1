  ...
  properties: {
    ...
    socialLinks: {
        ...
        items: {
            type: Type.OBJECT,
            properties: {
                platform: { type: Type.STRING, description: "e.g., LinkedIn, GitHub, Portfolio" },
    ...
    totalExperienceYears: { type: Type.NUMBER, description: "Calculated total years of professional experience based on work history." }
  },
  required: ["fullName", "email", "skills", "experience"]
};

// Input can be a raw string (from txt/docx) OR a file object (from pdf)
export const parseResumeText = async (input: string | { base64: string; mimeType: string }): Promise<ResumeData> => {
  try {
    let contents;
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        systemInstruction: "You are an expert HR Resume Parser. Extract accurate information. If a field is missing, use an empty string or empty array."
      }
    });
...
export const generateInterviewQuestions = async (candidate: Candidate, jobDescription?: string): Promise<InterviewQuestion[]> => {
  try {
    const context = `
        ...
        responseSchema: {
            ...
            items: {
                ...
                properties: {
                    ...
                    type: { type: Type.STRING, enum: ['technical', 'scenario', 'project', 'behavioral'] },
                    context: { type: Type.STRING, description: "Why this question is relevant" },
                    answerGuide: { type: Type.STRING, description: "Key points of a good answer" }
                }
            }
        }
      }
...
export const analyzeCandidateMatch = async (candidate: Candidate, jobRole: JobRole): Promise<{ score: number; analysis: string; matchedSkills: string[]; missingSkills: string[] }> => {
  try {
    const prompt = `
